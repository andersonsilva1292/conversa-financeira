          ...
          <Dialog open={showForm} onOpenChange={(open) => { setShowForm(open); if (!open) { setEditingGoal(null); setForm({ title: "", target_amount: "", current_amount: "0", deadline: "", icon: "🎯" }); } }}>
            ...
            <DialogContent className="bg-card border-border">
              ...
              <div className="space-y-4 mt-2">
                <input value={form.title} onChange={e => setForm(f => ({ ...f, title: e.target.value }))} placeholder="Título da meta" aria-label="Título da meta" className="w-full bg-secondary text-foreground placeholder:text-muted-foreground px-4 py-3 rounded-lg text-sm outline-none border border-border focus:border-primary/50" />
                <input type="number" step="0.01" value={form.target_amount} onChange={e => setForm(f => ({ ...f, target_amount: e.target.value }))} placeholder="Valor objetivo (R$)" aria-label="Valor objetivo da meta" className="w-full bg-secondary text-foreground placeholder:text-muted-foreground px-4 py-3 rounded-lg text-sm outline-none border border-border focus:border-primary/50" />
                <input type="number" step="0.01" value={form.current_amount} onChange={e => setForm(f => ({ ...f, current_amount: e.target.value }))} placeholder="Valor atual (R$)" aria-label="Valor atual da meta" className="w-full bg-secondary text-foreground placeholder:text-muted-foreground px-4 py-3 rounded-lg text-sm outline-none border border-border focus:border-primary/50" />
                <input value={form.deadline} onChange={e => setForm(f => ({ ...f, deadline: e.target.value }))} placeholder="Prazo (ex: Jun 2026)" aria-label="Prazo da meta" className="w-full bg-secondary text-foreground placeholder:text-muted-foreground px-4 py-3 rounded-lg text-sm outline-none border border-border focus:border-primary/50" />
